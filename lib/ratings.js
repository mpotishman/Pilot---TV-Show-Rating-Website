  ...
  return db
    .prepare(
      `INSERT INTO season_ratings
        (user_id, show_id, season_number, average_rating, season_review, updated_at)
       VALUES (?, ?, ?, ?, ?, strftime('%s','now'))
       ON CONFLICT(user_id, show_id, season_number)
       DO UPDATE SET
         average_rating = excluded.average_rating,
         season_review = excluded.season_review,
         updated_at = strftime('%s','now')`
    )
...
export async function getRatingInformation(user_id) {
  ...
  const normalizedEpisodes = episodeRatings.map(r => ({ type: "episode", ...r }))
  const normalizedSeasons = seasonRatings.map(r => ({ type: "season", ...r }))
  ...
  combined.sort((a, b) => Number(b.created_at) - Number(a.created_at))
