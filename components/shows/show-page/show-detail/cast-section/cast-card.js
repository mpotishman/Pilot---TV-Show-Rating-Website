...
export default function CastCard({ castInfo }) {
  const castUrl = castInfo?.profile_path
    ? `https://image.tmdb.org/t/p/w780${castInfo.profile_path}`
    : "/images/hero.jpg";

    const character = castInfo.roles.map((char) => 
      char.character).join(", ");
